...
async function generateMarkmapSyntax(
  ...
  const response = await openAIService.completion({
    messages: [
      {
        role: "system",
        content: `This transcription was generated from dictated audio and may contain imperfections. 
        Convert it into a valid markmap syntax, focusing on the most crucial concepts and including necessary additional notes. 
        Remove background noise and unrelated fragments. Begin your response with a *thinking* block to reflect on structuring and presentation.
         Then, use <final_result> tags to enclose the markmap syntax without any additional formatting. 
         Start the markmap content immediately after the opening tag.
...
async function main() {
  ...
  try {
    ...
    const markmapSyntax = await generateMarkmapSyntax(
      transcription,
      openAIService
    );
    ...
    console.log(
      "Mindmap HTML generated successfully in the mindmap directory."
    );
  } catch (error) {
